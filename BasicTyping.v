...
| BtFix : forall Γ (Tx: base_ty) T e (L: aset),
    (forall (x: atom), x ∉ L -> tm_has_type (<[x := TBase Tx]>Γ) (vlam (Tx ⤍ T) e ^^ x) ((Tx ⤍ T) ⤍ T)) ->
    value_has_type Γ (vfix (Tx ⤍ T) (vlam (Tx ⤍ T) e)) (Tx ⤍ T).
...
#[global]
Instance has_normal_type_tm_instance : Typing context tm ty := tm_has_type.
Arguments has_normal_type_tm_instance /.

#[global]
Instance has_normal_type_value_instance : Typing context value ty := value_has_type.
